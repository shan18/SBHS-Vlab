...
def add_question(request, question_id=None):
    ...
    if request.method == 'POST':
        ...
        if qform.is_valid():
            ...
            return my_render_to_response(request, "yaksh/add_question.html", context,
                                         context_instance=ci)
    ...
    context = {'qform': qform, 'fileform': fileform, 'question': question,
               'formsets': formsets, 'uploaded_files': uploaded_files}
    return my_render_to_response(request, "yaksh/add_question.html", context, context_instance=ci)
    ...
    if request.method == "POST":
        if quiz_id is None:
            ...
            if form.is_valid():
                ...
                context["form"] = form
                return my_render_to_response(request, 'yaksh/add_quiz.html',
                                             context,
                                             context_instance=ci)
        else:
...
def add_course(request, course_id=None):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return my_render_to_response(request, 'yaksh/add_course.html',
                                         {'form': form},
                                         context_instance=ci)
    else:
        form = CourseForm(instance=course)
        return my_render_to_response(request, 'yaksh/add_course.html', {'form': form},
                                     context_instance=ci)
...
def show_statistics(request, questionpaper_id, attempt_number=None):
    ...
    if attempt_number is None:
        ...
        return my_render_to_response(request, 'yaksh/statistics_question.html', context,
                                     context_instance=RequestContext(request))
